...
# cat("Estat?stica do Teste: ", Q.B, "\n")
# cat("n?vel descritivo: ",e.nd.QB,"\n")
# cat("Matrizes de Covari?ncias por grupo: \n")
...
# cat("Matriz M :","\n")
# print(m.M)
p.valor <- round(p.valor,3)
p.valor <- ifelse(p.valor == 0,"< 0,001",p.valor)
data.frame(Est = round(estat,2), pvalor = p.valor)
}
